...
from templanator.code_examples import CodeGenerator
...
class TemplateTester(QMainWindow):
    ...
    def init_template_engines(self):
        ...
        self.mode_combo.addItem("--- Select Example ---")
        self.mode_combo.addItems(self.engines.keys())
        self.mode_combo.currentTextChanged.connect(self.on_mode_changed)
        self.mode_combo.currentTextChanged.connect(self.load_example)
    ...
    def init_ui(self):
        ...
        code_label = QLabel("Example Code")
        ...
        main_layout.addWidget(code_panel)
    ...
    def load_example(self):
        """Load example for the current template engine, including code example"""
        current_mode = self.mode_combo.currentText()
        if not current_mode:
            return
        ...
        source = ""
        # Get source and template from the appropriate example source
        if engine_type in ('textfsm', 'ttp'):
            ...
            # Fetch the corresponding code example
            code = CodeGenerator.get_example_data(engine_type)[1]
        else:  # Jinja2
            source, template = engine.get_example()
            code = ""  # No predefined code example for Jinja2

        # Set UI elements
        self.source_text.setPlainText(source)
        self.template_text.setPlainText(template)
        self.code_text.setPlainText(code)  # Now correctly setting the code example

        # Clear result field when loading a new example
        self.result_text.clear()
