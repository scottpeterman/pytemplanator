...
class SimplifiedTTPEngine:
    ...
    def find_best_template(self, output, filter=None):
        best_template = None
...
PID: PWR-7200-AC       , VID:    , SN:
    ...
    best_template, best_parsed_output, best_score = ttp_engine.find_best_template(input_text, filter="inventory")

    # Display the results
    if best_template:
        print(f"\nBest Template: {best_template}")
        print("Parsed Content:", json.dumps(best_parsed_output, indent=2))
        print(f"Template Score {best_score}")
    else:
